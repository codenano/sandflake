...
var Thread = require('./thread'),
    thread = new Thread(sessionStore),
    mail = require('./mail'),
    fs = require('fs'),
  ...
  exports.room.on('connection', function(ws) {
    ...
    ws.on('message', function(data){
        ...
        case 'meat':
           ...
           thread.setMsg(data, function(err, res){
             if (!err)
              ...
              sessionStore.db.get('\xff'+'session:'+sessionID+'\xff'+'uname', function(err, sess) {
                if (!err) {
                    ...
                    if (!(_.find(roomUS[data.room], function(us){return us === uname}))){
                       ...
                       if (data.room === 'main')
                          _.each(roomsWS[data.room], function(client){
                             client.send(JSON.stringify({type: 'room:main', response: roomUS[data.room]}));
                          });
                       else {
        ...
        case 'leave':
          console.log('leaving @'+data.room);
            ...
            cookieParser(ws.upgradeReq, null, function(err) {
              ...
              _.reject(roomsWS['main'], function(client){
              return (client === ws)
              });
              if (data.room === 'main')
                _.each(roomsWS[data.room], function(client){
        ...
        case 'signOut':
          cookieParser(ws.upgradeReq, null, function(err) {
             ...
             if (sess) {
             var uname = JSON.parse(sess.toString('utf8'));
             console.log('loging out '+uname+' '+roomUS['main']);
             roomUS['main'] = _.without(roomUS['main'], uname);
             _.each(roomsWS['main'], function(client){
              try {
               client.send(JSON.stringify({type: 'room:main', response: roomUS['main']}));
